...
**The ~9% dollar undercount is FIXED as of skill-templates#259 (merged 2026-09-20),
but every figure recorded BEFORE it still carries the error.** The cause was never
arithmetic. Claude Code writes some assistant messages to the transcript **twice under
one `requestId`** — a partial record first, the complete one minutes later — and the
old dedup key `(message id, requestId)` kept the FIRST and discarded the completed
record. Measured 2026-09-16: first-occurrence $2,499 against complete-record $2,746
over 20,088 requests since the reset, 9,936 of them duplicated. The script now bills
the complete record and supersedes the partial, in the per-minute index as well as the
totals.

**What that does not repair is the ledger.** Every reading in `meter-readings.md`,
every benchmark row, and every cap calibrated before 2026-09-20 was measured with the
old policy and reads roughly 9% LOW — a corrected figure and a historical one are not
comparable, and a cap derived from the two together is wrong in a direction nothing
announces. Note the policy a figure was measured under before differencing it against
a newer one. Pacing decisions taken under the old script were nonetheless sound: the
ratios cancelled, so $/pt, the headroom in points, the landing and the time to the
wall divided one understated total by another and were first-order right.
