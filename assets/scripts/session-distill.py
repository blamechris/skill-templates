...
`session-distill.json`'s `records[]`:
  ...
  run                   provenance + shape, NOT the full brief/report text
                         ...
                         tool_calls, and (main-turn only) segmented_by.
  asked / understood /
  ...
  claims[]              the join column: {id, text, kind, proof, quote}.
                         ...
                         label auditable rather than a bare string.
...
def build_tool_trace(objs):
    """(tool_calls_count, trace[]) over a span of already-parsed transcript
    lines -- one entry per tool_use block on an assistant line, matched
    against a later tool_result (by tool_use_id) for `errored`."""
    error_map = {}
    for o in objs:
        ...
        for b in content:
            if isinstance(b, dict) and b.get("type") == "tool_use":
                trace.append({
                    ...
                    "errored": error_map.get(b.get("id"), False),
                })
...
def build_main_turn_stub(main_path, objs, start, end, idx, segmented_by):
    ...
    tool_calls, tool_trace = build_tool_trace(span)
...
def build_subagent_stub(rr, jsonl_path, unreadable):
    ...
    tool_calls, tool_trace = build_tool_trace(objs)
...
def cmd_runs(a):
    ...
    if a.json:
        ...
        for r in runs:
            print("  %-18s %-10s %-10s brief=%-6d report=%-6d tools=%d  %s" % (
