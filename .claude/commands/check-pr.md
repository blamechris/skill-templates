...
Copilot review is optional for this repo. If no Copilot review is configured, skip to step 1. Otherwise, Copilot review typically takes **3-5 minutes** after PR creation to even begin. If you run `/check-pr` immediately after creating the PR, the review won't exist yet.

**IMPORTANT:** Do NOT skip this step if Copilot review is enabled. If no Copilot review exists and the PR was created recently (within 5 min), you MUST wait — otherwise you'll process zero comments and miss the entire review.
