...
3. **Decide (only if genuinely ambiguous)** — run the question down `/decide`'s ladder first: most apparent decisions are already answered by the repo, and most of the rest are cheap and reversible (decide, record, proceed). For what survives that — a real decision (epic scope, design fork, choosing among N approaches) — run the decision panel ({{CUSTOMIZE: `/swarm-audit` or equivalent}}), **pick the recommended option**, and **record the decision** in the session log plus a one-line note on the issue. Never block on the user.
...
- **Wave handoff note:** {{CUSTOMIZE: handoff-note path — default `docs/handoffs/YYYY-MM-DD-<slug>.md`}}. The session-boundary seed: queue position, open blockers, the awaiting-user list, the last verified merge. It is **disposable by design** — everything in it is re-derivable from GitHub — a seed for the next wave's session, never a second source of truth. Each wave boundary **adds a dated file**; it never rewrites an earlier one, and a relaunch task must name that exact dated file, never a pointer that can move. Two conditions make it real, and neither is optional:
  - **It carries `/session-lifecycle`'s End-step-1 frontmatter** — `type: handoff`, `date:` as a full unquoted UTC timestamp (`2026-08-11T21:40Z`), `repo:`, `picks_up_at:`, `sensitivity:` — with a `YYYY-MM-DD-<slug>.md` name. Several waves end on one day, so the timestamp, not the filename date, is what orders them; a headerless seed does not lose quietly, it forces the next session to disambiguate by hand.
  - **It is committed and pushed the moment it is written** — `git add docs/handoffs/<file>` by name, commit, push, then `git cat-file -e "origin/$(git branch --show-current):docs/handoffs/<file>"`. Until that succeeds the seed is an untracked file in this wave's worktree and nowhere else, and `git worktree remove --force` deletes it silently. **No worktree teardown and no session end before it succeeds.**
...
- **Handoff-note path** — where wave handoff notes live, default `docs/handoffs/YYYY-MM-DD-<slug>.md`, committed, with the `/session-lifecycle` End-step-1 frontmatter (State).
