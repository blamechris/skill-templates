...
- **Default compile targets** — `scripts/compile-skill-targets.mjs` reads the `targets:` line
  ...
  codex`); codex emits a repo-tracked `.codex/skills/<name>/` folder, so including it is safe by
  default. With no `targets:` line the compiler falls back to `claude` only.
