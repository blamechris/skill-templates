...
- **Shed context at each wave boundary — mode-aware.** When a wave completes (after Phase 2 replenishment and the Phase 4 convergence check), write a **new** dated handoff note ({{CUSTOMIZE: handoff-note path — default `docs/handoffs/YYYY-MM-DD-<slug>.md`}}) — add a file, never rewrite the previous wave's — **then commit and push it before doing anything else** (below), and refresh the durable queue ({{CUSTOMIZE: queue path — default `scratchpad/autonomous-queue.json`}}), update the STATE header, then:
  ...
  - **Unattended with a configured re-launcher** ({{CUSTOMIZE: wave re-launcher — e.g. chroxy scheduled trigger, cron/launchd job, /loop wrapper; leave "none" if absent}}) — end the session; the re-launcher starts the next wave from the same seeds, and its task text **names the dated handoff file explicitly, never a pointer that can move**.
...
- **A wave seed is not written until it is committed — and it carries the header the selector reads.** Two non-optional halves, both of them load-bearing:

  **① The header.** A marathon ends several waves on one day, so the filename date cannot order them and the frontmatter timestamp is the only thing that can. Write the header `/session-lifecycle` End step 1 prescribes, verbatim in shape — `type: handoff`, `date:` as a **full unquoted UTC timestamp** (`2026-08-11T21:40Z`), `repo:`, `picks_up_at:`, `sensitivity:` — and name the file `YYYY-MM-DD-<slug>.md` with zero-padded fields. A seed with no header, or a name in any other shape, is one the newest-seed rule cannot place: it does not lose quietly, it forces the next session to stop and disambiguate by hand.
  ...
  **② The commit.** Immediately after writing it, before the queue refresh and before anything else touches git:
  ...
  git commit -m "docs(handoffs): record the wave <n> boundary"
  git push
  git cat-file -e "origin/$(git branch --show-current):docs/handoffs/<file>" && echo durable
  ```

  Until `durable` prints, the seed is an untracked file in this wave's worktree and nowhere else, and `git worktree remove --force` — the fleet's standard teardown — deletes it silently and unrecoverably. **Never tear down a wave's worktree, and never end the session, before that line prints.** A wave that ends with an uncommitted seed has not shed context, it has dropped the marathon's continuity.
...
- **Handoff-note path** — where wave handoff notes live, default `docs/handoffs/YYYY-MM-DD-<slug>.md` (committed, with the `/session-lifecycle` End-step-1 frontmatter)
