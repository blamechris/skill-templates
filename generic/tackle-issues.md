...
- **The wave seed is written outside every worktree, and it archives rather than overwrites.** Both halves come from `/session-lifecycle` End step 1 and neither is optional:
  ...
  **① The path.** `$CLAUDE_HANDOFF_DIR/NEXT-<scope>.md`, where `<scope>` is the main worktree's directory basename from `git rev-parse --path-format=absolute --git-common-dir` — the same key from every wave's worktree, so a marathon's waves all read and write one seed. Never inside a wave's worktree: `git worktree remove --force` deletes untracked files silently, and a wave that loses its seed has not shed context, it has dropped the marathon's continuity. Outside every workspace there is nothing to commit, nothing to push, and nothing to gate teardown on.

  **② The header, and archive-on-collide.** Write the frontmatter End step 1 prescribes — `type`, `date` (full UTC timestamp), `scope`, `session`, `picks_up_at`, `sensitivity`. If the canonical file already exists carrying a **different** `session:` — another session ended on this repo while the marathon was running — rename it to `NEXT-<scope>.<UTC>-<sid>.md` before writing. Successive waves of one marathon share a session id only within a session; across a restart the new session archives the previous wave's seed, which is the intended record.
