...
| Ready | {{CUSTOMIZE: Labels that signal "ready to work on" — e.g., `ready-to-build`, or issues with `complexity:low`/`complexity:medium` and acceptance criteria in the body}} |
...
- {{CUSTOMIZE: Repo-specific planning files — e.g., `GDD.md` for game projects, `docs/architecture/` for complex systems}}
...
```bash
# {{CUSTOMIZE: Source file patterns to scan — e.g., "*.js", "*.ts", "*.gd", "*.kt"}}
...
# {{CUSTOMIZE: Dependency check command — e.g., "npm outdated", "pip list --outdated", etc.}}
```
...
- **Ready-to-work labels** — labels that signal an issue is ready to pick up (e.g., `ready-to-build`, or complexity-based like `complexity:low`/`complexity:medium`)
- **Blocked labels** — labels meaning "don't touch" (e.g., `blocked`, `wontfix`, `needs-design`)
- **Roadmap file locations** — repo-specific planning documents beyond the defaults (e.g., `GDD.md`, `docs/design/`)
- **Source file patterns** — file extensions to scan for TODOs (e.g., `*.js`, `*.ts`, `*.gd`, `*.kt`)
- **Priority signals** — repo-specific signals that boost priority (milestones, special labels)
- **Test runner command** — for coverage checking in Phase 4 fallback
- **Dependency check command** — for outdated/vulnerability checking in Phase 4 fallback
- **Audit focus areas** — what matters most when scanning (e.g., "mobile performance", "game balance", "API security")
