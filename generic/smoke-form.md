...
- **Dictation (progressive enhancement):** if `window.SpeechRecognition ||
  ...
  **"You are being heard" signal (#316):** show a strong live indicator under the
...
- **Per-item screenshot paste (#316):** the per-item notes accept a pasted
  (`Ctrl+V`) or dropped image — store it WITH that item's result, show a dark
  thumbnail frame on the item, and (online mode) include it in the POST to the
  intake. Guard each image to a sane cap (~2.5 MB decoded); oversize → a visible
  note, never a silent drop. Intake side: decode the base64 data-URL to a file
  under `<results>.jsonl.files/` and store a `{file,bytes,mime}` ref in the row
